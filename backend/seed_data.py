...
def seed():
    ...
    for o in organizers:
        db.refresh(o)
    print(f"   [OK] {NUM_ORGANIZERS} organizers created")
    ...
    for i in range(NUM_VOLUNTEERS):
        ...
        vol = User(
            ...
            tasks_completed_count=random.randint(0, 50),
            timeliness_rating=round(random.uniform(0.5, 1.0), 2),
            performance_score=round(random.uniform(0, 500), 1),
        )
        db.add(vol)
    ...
    for v in volunteers:
        db.refresh(v)
    print(f"   [OK] {NUM_VOLUNTEERS} volunteers created")
    ...
    for t in tasks:
        db.refresh(t)
    print(f"   [OK] {NUM_TASKS} tasks created")
    ...
    db.commit()
    print(f"   [OK] {created_parts} participations created")
    ...
    for vol in volunteers:
        ...
        if comp_count >= 5:
            ...
            badge_count += 1
    db.commit()
    print(f"   [OK] {badge_count} badges awarded")
