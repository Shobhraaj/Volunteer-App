...
def update_reliability(db: Session, user: User) -> float:
    ...
    reliability = completed / (completed + no_show + cancelled_by_volunteer)
    """
    completed = db.query(Participation).filter(Participation.volunteer_id == user.id, Participation.status == "completed").count()
    no_shows = db.query(Participation).filter(Participation.volunteer_id == user.id, Participation.status == "no_show").count()
    cancelled = db.query(Participation).filter(Participation.volunteer_id == user.id, Participation.status == "cancelled").count()
    
    total = completed + no_shows + (cancelled * 0.5)  # Cancelled counts as half a penalty
    if total == 0:
...
def calculate_performance_score(user: User) -> float:
    """
    Calculate overall performance score (0-100).
    Factors: 
      - Tasks Completed (40%)
      - Reliability/Success Rate (40%)
      - Point accumulation (20%)
    """
    completed_score = min(100, (user.tasks_completed or 0) * 5) # Maxes at 20 tasks
    reliability_score = (user.reliability_score or 1.0) * 100
    points_score = min(100, (user.points or 0) / 10) # Maxes at 1000 points
    
    final_score = (completed_score * 0.4) + (reliability_score * 0.4) + (points_score * 0.2)
