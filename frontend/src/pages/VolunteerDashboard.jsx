      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div className="animate-slide-up">
          <h1 className="text-4xl font-extrabold tracking-tight text-slate-900 dark:text-white mb-2">
            Welcome back, <span className="text-primary-500">{user?.full_name?.split(' ')[0]}</span> 👋
          </h1>
          <p className="text-slate-500 dark:text-slate-400 font-medium">
            Here's your volunteer activity and AI-powered task recommendations
          </p>
        </div>
        <div className="flex items-center gap-4 p-4 glass-card shadow-sm animate-slide-up" style={{ animationDelay: '0.1s' }}>
          <div className="text-xs font-black uppercase tracking-widest text-slate-400">Your Status</div>
      ...
      {badges.length > 0 && (
        <div className="card mb-8 animate-slide-up" style={{ animationDelay: '0.2s' }}>
          ...
          <div className="flex gap-3 flex-wrap">
            {badges.map((b,i) => (
              <div key={i} className="px-4 py-2 bg-violet-500/10 border border-violet-500/20 rounded-xl text-sm font-bold text-violet-600 dark:text-violet-400 flex items-center gap-2 apple-hover">
                ...
                <div className="rec-card apple-hover group cursor-pointer" key={rec.task.id} onClick={() => navigate(`/tasks/${rec.task.id}`)}>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap mb-4">
                      {(rec.task.required_skills||[]).map(s => <span key={s} className="skill-tag">{s}</span>)}
                    </div>
                  ...
                  <div className="flex flex-col justify-end">
                    {!participation ? (
                      ...
                      <span className={`badge badge-${participation.status === 'completed' ? 'emerald' : 'violet'} py-2 px-4 !text-sm`}>{participation.status}</span>
                    )}
                  </div>
                </div>
              );
              ...
              return (
                <div className="card flex flex-col apple-hover group cursor-pointer" key={task.id} onClick={() => navigate(`/tasks/${task.id}`)}>
                  ...
                  <div className="flex flex-wrap mb-6">
                    {(task.required_skills||[]).slice(0, 3).map(s => <span key={s} className="skill-tag !text-[10px] !px-2">{s}</span>)}
                  ...
                  <tbody>
                    {history.slice(0,10).map((h) => (
                      <tr key={h.id}>
                        ...
                        <td>
                          <span className={`badge badge-${h.status==='completed'?'emerald':h.status==='no_show'?'danger':h.status==='assigned'?'cyan':'violet'}`}>
                            {h.status}
                          </span>
                        </td>
                        ...
                        <td>
                          {h.status === 'applied' && (
                            <button className="text-red-500 hover:text-red-600 font-bold text-xs uppercase" onClick={() => handleCancel(h.id)}>Withdraw</button>
                          )}
                        </td>
                      </tr>
                    ))}
